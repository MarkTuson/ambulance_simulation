...
def get_delay_period(time, params, secondary=False):
    ...
    for i in range(params["n_factors" + sffx]):
        if (
            time >= params["delay_split" + sffx][i]
            and time < params["delay_split" + sffx][i + 1]
        ):
            return i
...
def expected_pickup(initial_call_time, a, p, params, secondary=False):
    ...
    pick_up_transit_time = journey_time(
        initial_call_time, params["amb_to_patient"][a][p], params, secondary
    )
...
def get_service(initial_call_time, ind, params):
    ...
    expected_pick_up_transit_time = journey_time(
        initial_call_time, params["amb_to_patient"][a][p], params
    )
    ind.expected_pick_up_time = expected_pick_up_transit_time
    pick_up_transit_time = (
        ciw.random.triangular(0.75, 1.25, 1) * expected_pick_up_transit_time
    )
    ind.pick_up_time = pick_up_transit_time
    initial_call_time += pick_up_transit_time

    delay_at_site = (
        ciw.random.lognormvariate(
            params["delay_at_site"][0], params["delay_at_site"][1]
        )
        / 24
    )
    ...
    hospital = ciw.random_choice(
        list(range(params["n_hospitals"])) + [-1], hosp_probs + [1 - sum(hosp_probs)]
    )
    ...
    if hospital == -1:
        ...
        expected_return_to_loc_time = journey_time(
            initial_call_time, params["patient_to_amb"][p][a], params
        )
        return_to_loc_time = (
            ciw.random.triangular(0.75, 1.25, 1) * expected_return_to_loc_time
        )
        ...
        ind.complete_time = (
            ind.pick_up_time
            + ind.delay_at_site
            + ind.return_to_loc_time
            + ind.refill_time
        )

    else:
        ind.hospital = hospital
        expected_to_hosp_transit_time = journey_time(
            initial_call_time, params["patient_to_hosp"][p][hospital], params
        )
        to_hosp_transit_time = (
            ciw.random.triangular(0.75, 1.25, 1) * expected_to_hosp_transit_time
        )
        initial_call_time += to_hosp_transit_time
        ind.to_hospital_time = to_hosp_transit_time

        delay_at_hospital = ciw.random.uniform(
            params["delay_at_hosp"][k][0], params["delay_at_hosp"][k][1]
        )
        initial_call_time += delay_at_hospital
        ind.delay_at_hospital = delay_at_hospital

        expected_return_to_loc_time = journey_time(
            initial_call_time, params["hosp_to_amb"][hospital][a], params
        )
        return_to_loc_time = (
            ciw.random.triangular(0.75, 1.25, 1) * expected_return_to_loc_time
        )
        ...
        ind.complete_time = (
            ind.pick_up_time
            + ind.delay_at_site
            + ind.to_hospital_time
            + ind.delay_at_hospital
            + ind.return_to_loc_time
            + ind.refill_time
        )
        ...
        arrival_distributions={
            ...
            + str(c): [
                ciw.dists.PoissonIntervals(
                    rates=r, endpoints=params["demand_split"], max_sample_date=max_time
                )
            ]
...
class ResponseJob(ciw.Individual):
    def __init__(
        self, id_number, row, customer_class=0, priority_class=0, simulation=False
    ):
        super().__init__(id_number, customer_class, priority_class, simulation)
...
def get_service_response(initial_call_time, ind, params):
    ...
    expected_pick_up_transit_time = journey_time(
        initial_call_time, params["amb_to_patient"][a][p], params, secondary=True
    )
    ind.expected_pick_up_time = expected_pick_up_transit_time
    if ind.expected_pick_up_time > ind.expected_ambulance_pick_up_time:
        ...
        ind.delay_at_site = max(
            ind.ambulance_pick_up_time + ind.ambulance_delay_at_site - ind.pick_up_time,
            0,
        )

        expected_return_to_loc_time = journey_time(
            initial_call_time, params["patient_to_amb"][p][a], params, secondary=True
        )
        return_to_loc_time = (
            ciw.random.triangular(0.75, 1.25, 1) * expected_return_to_loc_time
        )
...
def create_response_network(params, initial_recs):
    arrivals = (
        [list(initial_recs["call_date"])[0]]
        + list(initial_recs["call_date"].diff())[1:]
        + [float("inf")]
    )
    N = ciw.create_network(
        arrival_distributions=[ciw.dists.Sequential(arrivals)]
        + [ciw.dists.NoArrivals() for _ in range(params["n_ambulances"])],
        service_distributions=[ciw.dists.Deterministic(0)]
        + [make_response_service_dist(params) for _ in range(params["n_ambulances"])],
...
def run_full_simulation(params, max_time, trial, progress_bar=False):
    ...
    initial_recs = (
        recs_transit[
            (recs_transit["ambulance_id"] != "LFalse VFalse")
            & (recs_transit["speciality"] != 2)
        ]
    ...
    recs["response_time"] = recs[["ambulance_pick_up_time", "rrv_pick_up_time"]].min(
        axis=1
    )
