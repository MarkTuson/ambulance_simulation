...
if __name__ == "__main__":
    ...
    neighbourhood_to_amb = np.genfromtxt(
        f"src/data/posts/original/neighbourhood_to_amb.csv", delimiter=","
    )
    ...
    amb_to_neighbourhood = np.genfromtxt(
        f"src/data/posts/original/amb_to_neighbourhood.csv", delimiter=","
    )
    ...
    demand_A1 = pd.read_csv(
        f"src/data/demand_34/A1_demand.csv", index_col=0, names=range(4)
    )
    demand_A2 = pd.read_csv(
        f"src/data/demand_34/A2_demand.csv", index_col=0, names=range(4)
    )
    demand_B = pd.read_csv(
        f"src/data/demand_34/B_demand.csv", index_col=0, names=range(4)
    )
    ...
    allocation_primary = np.genfromtxt(
        f"src/data/posts/original/allocations/demand_34_withRRV_{rl}.csv", delimiter=","
    )
    allocation_secondary = np.genfromtxt(
        f"src/data/posts/original/allocations_secondary/demand_34_withRRV_{rl}.csv",
        delimiter=",",
    )
    ...
    delay_factor = [
        np.genfromtxt(f"src/data/global/2019/delay_factor.csv", delimiter=",").mean()
    ]
    delay_factor_secondary = [
        np.genfromtxt(
            f"src/data/global/2019/delay_factor_secondary.csv", delimiter=","
        ).mean()
    ]
