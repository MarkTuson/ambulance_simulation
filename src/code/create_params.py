...
if __name__ == "__main__":
    ...
    neighbourhood_to_amb = np.genfromtxt(f'src/data/posts/{posts}/neighbourhood_to_amb.csv', delimiter=',')
    neighbourhood_to_amb = adjust_travel_time_matrix(neighbourhood_to_amb, 5, 1440)

    ## AMBULANCE TO NEIGHBOURHOOD Travel Times
    amb_to_neighbourhood = np.genfromtxt(f'src/data/posts/{posts}/amb_to_neighbourhood.csv', delimiter=',')
    ...
    demand_A1 = pd.read_csv(f'src/data/demand_{demand}/A1_demand.csv', index_col=0, names=range(len(demand_split)))
    demand_A2 = pd.read_csv(f'src/data/demand_{demand}/A2_demand.csv', index_col=0, names=range(len(demand_split)))
    demand_B = pd.read_csv(f'src/data/demand_{demand}/B_demand.csv', index_col=0, names=range(len(demand_split)))
    ...
    # Get allocation
    allocation_primary = np.genfromtxt(f'src/data/posts/{posts}/allocations/{allocation}.csv', delimiter=',')
    allocation_secondary = np.genfromtxt(f'src/data/posts/{posts}/allocations_secondary/{allocation}.csv', delimiter=',')
